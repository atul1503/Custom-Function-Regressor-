...
def ActualCall():
    ...
    print()
    return LinearRegressor(data,d,fileName,maxe,it,lookup,interval,load)
...
def direction(y,x,lookup,d,w,k):
    ...
    if abs(pos)<abs(neg):
        ...
        final=0
        print('The regressor hit a local minima with weights:',w,'\nI know! I too hate when that happens.')
        input()
        exit()
...
def LinearRegressor(data,d,fileName,maxe,it,lookup,interval,load):
    w=[0.01]*weights
    mvp=0
    flag=0
    e=1e+100
